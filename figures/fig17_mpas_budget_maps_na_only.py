"""MPAS current vs future LWA-budget composite maps, NA storms only
(paper Fig. 17)."""
...
def main(
        ...
        figure_name: Annotated[str, typer.Option()] = (
            "fig17_mpas_budget_maps_cf_na_only_recurvature.png"),
    ...
    pr_note = ("(%(letter)s) MPAS precipitation (mm hr$^{-1}$)"
               if has_mpas_pr
               else "(%(letter)s) MPAS precipitation (mm hr$^{-1}$) "
                    "— missing")
    ...
    lh_title = ("(%(letter)s) MPAS LH-LWA anomaly (T$-$pre)"
                if has_mpas_lh
                else "(%(letter)s) MPAS LH-LWA — missing ``mpas_lh_lwa``")
    ...
    budget_maps.plot_budget_wp_na_fig5(
        ...
        strat_column_titles=(
            f"MPAS current (NA only, N={n_c})",
            f"MPAS future (NA only, N={n_f})",
        ),
