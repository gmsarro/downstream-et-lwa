"""LWA-based RWP frequency/amplitude climatology plus raw-LWA climatology from filtered-LWA files."""
...
FILTERED_FILENAME_PATTERN = "lwa_filtered_{year}_{month:02d}.nc"
...
class _SeasonAccumulation:
    ...
    envf2d_sum: np.ndarray
    lwaraw2d_sum: np.ndarray
    mask1d_sum: np.ndarray
    envf1d_sum: np.ndarray
    envf1d_mask_sum: np.ndarray
    lwaraw1d_sum: np.ndarray
    n_times: int
...
def accumulate_season(*, season_months: list[int], year_start: int, year_end: int,
    ...
    envf2d_sum = np.zeros((NLAT, NLON), dtype=np.float64)
    lwaraw2d_sum = np.zeros((NLAT, NLON), dtype=np.float64)

    mask1d_sum = np.zeros(NLON, dtype=np.float64)
    envf1d_sum = np.zeros(NLON, dtype=np.float64)
    envf1d_mask_sum = np.zeros(NLON, dtype=np.float64)
    lwaraw1d_sum = np.zeros(NLON, dtype=np.float64)
    n_times = 0

    for year in range(year_start, year_end + 1):
        for month in season_months:
            path = input_directory / FILTERED_FILENAME_PATTERN.format(year=year, month=month)
            if not path.exists():
                continue
            with netCDF4.Dataset(path, "r") as d:
                lwa_raw = np.asarray(d["lwa_raw"][:])
                lwa_filt = np.asarray(d["lwa_filt"][:])
                hem = np.asarray(d["hem_mean_filt"][:])
            thr = (tau_star * hem)[:, None, None]
            mask_b = (lwa_filt > thr)
            mask = mask_b.astype(np.float32)
            lwa_filt_thr = np.where(mask_b, lwa_filt, 0.0).astype(np.float32)

            mask2d_sum += mask.sum(axis=0)
            envf2d_sum += lwa_filt_thr.sum(axis=0)
            lwaraw2d_sum += lwa_raw.sum(axis=0)

            e1d = (lwa_filt_thr[:, sel, :] * w[None, :, None]).sum(axis=1) / w_sum
            m1d = (e1d > 0).astype(np.float32)
            r1d = (lwa_raw[:, sel, :] * w[None, :, None]).sum(axis=1) / w_sum

            envf1d_sum += e1d.sum(axis=0)
            mask1d_sum += m1d.sum(axis=0)
            envf1d_mask_sum += (e1d * m1d).sum(axis=0)
            lwaraw1d_sum += r1d.sum(axis=0)
            n_times += mask.shape[0]

    return _SeasonAccumulation(
        mask2d_sum=mask2d_sum,
        envf2d_sum=envf2d_sum,
        lwaraw2d_sum=lwaraw2d_sum,
        mask1d_sum=mask1d_sum,
        envf1d_sum=envf1d_sum,
        envf1d_mask_sum=envf1d_mask_sum,
        lwaraw1d_sum=lwaraw1d_sum,
        n_times=n_times,
    )
...
def main(
    ...
    output_filename: Annotated[str, typer.Option()] = "lwa_climatology.nc",
) -> None:
    ...
    for season in season_names:
        ...
        agg = accumulate_season(season_months=months, year_start=year_start,
                                ...
                                tau_star=tau_star, input_directory=input_directory)
        n = agg.n_times
        if n == 0:
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            a2 = np.where(agg.mask2d_sum > 0,
                          agg.envf2d_sum / np.maximum(agg.mask2d_sum, 1),
                          0.0).astype(np.float32)
        raw2 = (agg.lwaraw2d_sum / n).astype(np.float32)

        f1 = (agg.mask1d_sum / n).astype(np.float32)
        with np.errstate(invalid="ignore", divide="ignore"):
            a1 = np.where(agg.mask1d_sum > 0,
                          agg.envf1d_mask_sum / np.maximum(agg.mask1d_sum, 1),
                          0.0).astype(np.float32)
        raw1 = (agg.lwaraw1d_sum / n).astype(np.float32)

        results[season] = _SeasonClimatology(
            freq_2d=f2, ampl_2d=a2, raw_2d=raw2,
            freq_1d=f1, ampl_1d=a1, raw_1d=raw1,
            n_times=n,
        )
        print(f"  n_times={n}  F_1d peak={f1.max()*100:.1f}%  "
              f"E_1d peak={a1.max():.2f} m/s  "
              f"rawLWA_1d peak={raw1.max():.2f} m/s")
    ...
    with netCDF4.Dataset(out_path, "w", format="NETCDF4") as out:
        ...
        for season, r in results.items():
            ...
            for name, arr, lname, unit, dims in [
                ("freq_2d", r.freq_2d,
                 f"LWA-based RWP frequency 2D ({season})", "1", ("lat", "lon")),
                ("ampl_2d", r.ampl_2d,
                 f"LWA-based RWP amplitude 2D ({season})",
                 "m s-1", ("lat", "lon")),
                ("raw_2d", r.raw_2d,
                 f"Raw LWA mean 2D ({season})",
                 "m s-1", ("lat", "lon")),
                ("freq_1d", r.freq_1d,
                 f"{season} 1D LWA-frequency (cos-wgt 20-80N per time)",
                 "1", ("lon",)),
                ("ampl_1d", r.ampl_1d,
                 f"{season} 1D LWA-amplitude (cos-wgt 20-80N per time)",
                 "m s-1", ("lon",)),
                ("raw_1d", r.raw_1d,
                 f"{season} 1D raw-LWA mean (cos-wgt 20-80N per time)",
                 "m s-1", ("lon",)),
            ]:
        ...
        out.source = str(input_directory) + "/lwa_filtered_YYYY_MM.nc"
